...
#################################################################################################################
def agg_loss_with_compact_filtering(loss_mat: torch.Tensor, response_mask: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    """
    Aggregate the loss matrix into a scalar.
    Args:
        loss_mat: `(torch.Tensor)`
    ...
    if loss_agg_mode == "token-mean":
        ...
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1) / torch.sum(response_mask, dim=-1)  # token-mean
        loss = torch.mean(seq_losses)  # seq-mean
    elif loss_agg_mode == "seq-mean-token-sum-norm":
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)
        loss = torch.sum(seq_losses) / loss_mask.shape[-1]  # The divisor
...
def compute_policy_loss_with_filter_mask(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    loss_mask = filter_mask * response_mask
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    pg_loss = agg_loss_with_compact_filtering(loss_mat=pg_losses, response_mask=response_mask, loss_mask=loss_mask, loss_agg_mode=loss_agg_mode)
