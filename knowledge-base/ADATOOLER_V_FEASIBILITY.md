...
## 8. Phase 0 后复审（2026-08-14，ccf-idea-reviewer 标准模式）

**加权分 3.70/5（此前 3.60），推荐 = revise（有条件继续）。** Phase 0 实证变化：锚点 149/191=78.0%（Gate 0 PASS）；verl-tool 环境全链可跑（原 Fallback #1 风险消除，可行性 5/5）；新证据：官方仓库 stale `llm_agent` import（"发布代码 ≠ 论文"叙事加强）；官方 delta_s 表与子集重叠仅 204/3000（Arm D 自算 ΔS，保真度声明须软化）；7B 跳过（上界引论文值）。

**新 prior art 压力（2026 检索新增，方向热度高于 8-13 评估）**：
| 工作 | 与我们重叠 | 差异化空间 |
|------|-----------|-----------|
| Diversity Over Frequency（ICML 2026, 2606.00096） | 已做工具奖励塑形消融（RFT/tool-ban/奖励加成/curiosity），结论：频率塑形失效、熵正则胜出 | 无 judge-ΔS 臂、无成本框架、非 V*/AdaTooler-V 管线、无检测协议 |
| HDPO "Act Wisely"（2604.08545） | 论证标量调用惩罚与 acc 奖励的优化矛盾（Arm B 的理论预判） | 我们是该判断的独立经验证据 + 规则信号零成本对比 |
| Beacon（2607.28595） | 自适应奖励（necessity-aware） | 无消融矩阵/成本量化 |
| TA-MDP 理论（2604.19857） | Reward Decomposition Theorem 触及 U/C/S 分解框架 | 我们可引用其界来解释分量消融 |
| VTool-R1（ICLR 2026） | 视觉工具 RL | 纯 outcome 奖励，无信号对比 |

**结论**：论文叙事必须前移「静默奖励死亡检测协议 + 发布代码保真审计 + 信号性价比框架」，「又一奖励消融」已不再成立（Arm B 若负结果将与 ICML 2026 结论互证，价值降级但非归零）。**Named blockers（写作前必修）**：B1 ΔS 保真度——用 204 行官方表重叠做校准对照（自算 ΔS vs 官方 ΔS 一致性即证据）；B2 单种子——胜者臂补 1 seed；B3 场地截止日核验（ICASSP 2027 / NAACL 2027，勿凭记忆）；B4 叙事重心前移。**分数变动条件**：Arm B 出现 calls↓ 且 acc 稳定 → +0.2~0.4；四臂全零且无检测协议新证据 → 落入 pivot 带（3.0-3.6），并入 Track B。置信度：medium（文献 1 天龄、场地截止日未核）。
