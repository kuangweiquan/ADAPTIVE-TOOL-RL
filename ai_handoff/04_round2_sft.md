...
## 远端补充 2026-08-10：v2 IoU 补算（离线，零成本）

评估脚本无 IoU 独立输出，已用 [compute_gt_iou.py](../experiments/scripts/compute_gt_iou.py) 从
trajectories_*.jsonl 离线补算（GT 取自 vstar_bench 各 sample 的 bbox，xywh→xyxy）。

**口径确认**：v1 历史 IoU>0.1 22% 与「口径 B（zoom 视图链逆映射回原图）样本级命中」22.0%（11/50）
精确吻合 → 历史口径即样本级任一调用命中。对比用口径 B。

| 口径 B（样本命中 IoU>0.1） | v1 | v2-强制 | v2-先答后验 | 门槛 |
|---|---|---|---|---|
| IoU>0.1 样本命中 | 22.0% (11/50) | **34.0% (17/50) ✅** | 34.0% (17/50) | ≥30% |
| 工具级命中 | 3.1% | 5.7% | 9.0% | — |
| 首调命中 | 22.0% | 34.0% | 34.0% | — |
| 平均最大 IoU | 0.083 | 0.123 | 0.120 | — |

（口径 A 记录坐标直比略高：v2-强制样本命中 42.0%，首调命中 34.0%，未用于对比。）

**结论**：IoU 维度 v2 达标（34% ≥ 30%，v1 22% → v2 34%）。注意首调命中 == 样本命中 == 34%：
模型第一步 zoom 就能覆盖目标的比例已从 22% 升到 34%，说明"定位"有改善；但工具级命中仅 5.7%
（逆映射口径）——死循环里的后续 zoom 映射回原图全是微小框，几乎不覆盖 GT，量化印证了
「扫描式平移」诊断。判定仍为**未通过**（准确率 34% < 50%，收敛 32% < 80%）。
